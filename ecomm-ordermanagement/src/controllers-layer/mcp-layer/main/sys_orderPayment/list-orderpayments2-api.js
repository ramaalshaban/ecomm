...
class ListOrderPayments2McpController extends OrderManagementMcpController {
  constructor(params) {
    super("listOrderPayments2", "listorderpayments2", params);
    this.dataName = "sys_orderPayments";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListOrderPayments2Manager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: ListOrderPayments2McpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListOrderPayments2McpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListOrderPayments2McpController.getOutputSchema().parse(result);
