...
class ListPaymentCustomersMcpController extends OrderManagementMcpController {
  constructor(params) {
    super("listPaymentCustomers", "listpaymentcustomers", params);
    this.dataName = "sys_paymentCustomers";
    this.crudType = "list";
  }

  createApiManager() {
    return new ListPaymentCustomersManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: ListPaymentCustomersMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new ListPaymentCustomersMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return ListPaymentCustomersMcpController.getOutputSchema().parse(result);
