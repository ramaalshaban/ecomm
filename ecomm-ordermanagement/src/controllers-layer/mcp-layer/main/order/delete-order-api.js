...
class DeleteOrderMcpController extends OrderManagementMcpController {
  constructor(params) {
    super("deleteOrder", "deleteorder", params);
    this.dataName = "order";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteOrderManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: DeleteOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeleteOrderMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeleteOrderMcpController.getOutputSchema().parse(result);
