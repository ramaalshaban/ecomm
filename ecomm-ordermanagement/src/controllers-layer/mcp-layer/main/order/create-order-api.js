...
class CreateOrderMcpController extends OrderManagementMcpController {
  constructor(params) {
    super("createOrder", "createorder", params);
    this.dataName = "order";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateOrderManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      adminNotes: z
        ...
        .max(255)
        .optional()
        .describe("Notes about the order (visible/editable by admins only)."),
...
module.exports = (headers) => {
  return {
    ...
    parameters: CreateOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CreateOrderMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CreateOrderMcpController.getOutputSchema().parse(result);
