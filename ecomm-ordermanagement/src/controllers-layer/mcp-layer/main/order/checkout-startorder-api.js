...
class CheckoutStartOrderMcpController extends OrderManagementMcpController {
  constructor(params) {
    super("checkoutStartOrder", "checkoutstartorder", params);
    this.dataName = "order";
    this.crudType = "update";
  }

  createApiManager() {
    return new CheckoutStartOrderManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      paymentUserParams: z
        ...
        .optional()
        .describe(
          "The user parameters that should be defined to start a stripe payment process",
        ),
    };
  }
}
...
module.exports = (headers) => {
  return {
    ...
    description: "Start checkout for order",
    parameters: CheckoutStartOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CheckoutStartOrderMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CheckoutStartOrderMcpController.getOutputSchema().parse(result);
