...
class GetOrderMcpController extends OrderManagementMcpController {
  constructor(params) {
    super("getOrder", "getorder", params);
    this.dataName = "order";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetOrderManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetOrderMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetOrderMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetOrderMcpController.getOutputSchema().parse(result);
