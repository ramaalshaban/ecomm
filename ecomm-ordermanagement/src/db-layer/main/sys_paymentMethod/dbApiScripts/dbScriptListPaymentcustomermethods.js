...
// This script is called from the ListPaymentCustomerMethodsManager
...
const dbScriptListPaymentcustomermethods = async (apiManager) => {
  ...
  try {
    ...
    let sys_paymentMethodList = await Sys_paymentMethod.findAll(options);

    const dbData = { items: [] };
    apiManager.sys_paymentMethods = [];
    if (!sys_paymentMethodList || sys_paymentMethodList.length == 0)
      return dbData;

    dbData.items = Array.isArray(sys_paymentMethodList)
      ? sys_paymentMethodList
      : [sys_paymentMethodList];
    ...
    apiManager.sys_paymentMethods = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListPaymentcustomermethods",
      {
...
module.exports = dbScriptListPaymentcustomermethods;
