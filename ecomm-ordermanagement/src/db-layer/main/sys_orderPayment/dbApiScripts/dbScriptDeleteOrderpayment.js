...
// This script is called from the DeleteOrderPaymentManager
...
const dbScriptDeleteOrderpayment = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Sys_orderPayment.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    apiManagersys_orderPayment = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteOrderpayment",
      {
...
module.exports = dbScriptDeleteOrderpayment;
