...
// This script is called from the UpdateOrderPaymentManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.sys_orderPayment;
  return {
    old_sys_orderPayment: oldData,
    sys_orderPayment: newData,
...
const dbScriptUpdateOrderpayment = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Sys_orderPayment.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateOrderpayment",
      {
...
module.exports = dbScriptUpdateOrderpayment;
