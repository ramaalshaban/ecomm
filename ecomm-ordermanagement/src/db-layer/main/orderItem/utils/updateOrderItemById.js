...
const updateOrderItemById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await OrderItem.findOne({ where: { id } });
    ...
    const [rowsCount, [dbDoc]] = await OrderItem.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateOrderItemById;
