...
// This script is called from the ListOrdersManager
...
const dbScriptListOrders = async (apiManager) => {
  ...
  try {
    ...
    const dbData = { items: [] };
    apiManager.orders = [];
    if (!orderList || orderList.length == 0) return dbData;

    dbData.items = Array.isArray(orderList) ? orderList : [orderList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.orders = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListOrders",
      {
...
module.exports = dbScriptListOrders;
