...
// This script is called from the DeleteOrderManager
...
const dbScriptDeleteOrder = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Order.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteOrder",
      {
...
module.exports = dbScriptDeleteOrder;
