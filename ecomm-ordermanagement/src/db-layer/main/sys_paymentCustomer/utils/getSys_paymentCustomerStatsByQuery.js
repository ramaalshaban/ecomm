...
const getSys_paymentCustomerStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          Sys_paymentCustomer.count({ where: queryWithSoftDelete }),
        );
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            Sys_paymentCustomer.sum(statParts[1], {
          ...
          promises.push(
            Sys_paymentCustomer.avg(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            Sys_paymentCustomer.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            Sys_paymentCustomer.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await Sys_paymentCustomer.count({ where: queryWithSoftDelete });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingSys_paymentCustomerStatsByQuery",
      err,
    );
  }
};

module.exports = getSys_paymentCustomerStatsByQuery;
