...
const updateSys_paymentCustomerById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Sys_paymentCustomer.findOne({
    ...
    const [rowsCount, [dbDoc]] = await Sys_paymentCustomer.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateSys_paymentCustomerById;
