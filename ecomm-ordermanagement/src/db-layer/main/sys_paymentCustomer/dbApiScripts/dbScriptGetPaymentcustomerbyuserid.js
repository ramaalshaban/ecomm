...
// This script is called from the GetPaymentCustomerByUserIdManager
...
const dbScriptGetPaymentcustomerbyuserid = async (apiManager) => {
  ...
  try {
    ...
    let rowData = await Sys_paymentCustomer.findOne(options);
    if (Array.isArray(rowData)) rowData = rowData[0];

    if (!rowData) return null;

    const dbData = rowData.getData();
    apiManager.sys_paymentCustomer = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptGetPaymentcustomerbyuserid",
      {
...
module.exports = dbScriptGetPaymentcustomerbyuserid;
