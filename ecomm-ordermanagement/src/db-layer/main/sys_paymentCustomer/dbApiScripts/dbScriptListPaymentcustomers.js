...
// This script is called from the ListPaymentCustomersManager
...
const dbScriptListPaymentcustomers = async (apiManager) => {
  ...
  try {
    ...
    let sys_paymentCustomerList = await Sys_paymentCustomer.findAll(options);

    const dbData = { items: [] };
    apiManager.sys_paymentCustomers = [];
    if (!sys_paymentCustomerList || sys_paymentCustomerList.length == 0)
      return dbData;

    dbData.items = Array.isArray(sys_paymentCustomerList)
      ? sys_paymentCustomerList
      : [sys_paymentCustomerList];
    ...
    apiManager.sys_paymentCustomers = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListPaymentcustomers",
      {
...
module.exports = dbScriptListPaymentcustomers;
