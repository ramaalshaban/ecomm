...
const { OrderpaymentDeletedPublisher } = require("../../api-events/publishers");
...
const { dbScriptDeleteOrderpayment } = require("dbLayer");

class DeleteOrderPaymentManager extends Sys_orderPaymentManager {
  ...
  async doBusiness() {
    const orderpayment = await dbScriptDeleteOrderpayment(this);
    return orderpayment;
  }
  ...
  async raiseEvent() {
    OrderpaymentDeletedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = DeleteOrderPaymentManager;
