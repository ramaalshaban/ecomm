...
const { dbScriptGetOrderpayment2 } = require("dbLayer");

class GetOrderPayment2Manager extends Sys_orderPaymentManager {
  ...
  async doBusiness() {
    const orderpayment2 = await dbScriptGetOrderpayment2(this);
  ...
  async raiseEvent() {
    Orderpayment2RetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetOrderPayment2Manager;
