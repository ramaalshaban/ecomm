...
const { dbScriptGetOrderpaymentbyorderid } = require("dbLayer");

class GetOrderPaymentByOrderIdManager extends Sys_orderPaymentManager {
  ...
  async getRouteQuery() {
    return { $and: [{ orderId: { $eq: this.orderId } }, { isActive: true }] };
  ...
  async doBusiness() {
    const orderpaymentbyorderid = await dbScriptGetOrderpaymentbyorderid(this);
  ...
  async raiseEvent() {
    OrderpaymentbyorderidRetrivedPublisher.Publish(
      this.output,
      this.session,
    ).catch((err) => {
...
module.exports = GetOrderPaymentByOrderIdManager;
