...
const { dbScriptCheckoutCompleteorder } = require("dbLayer");

class CheckoutCompleteOrderManager extends OrderManager {
  ...
  async doBusiness() {
    const completeorder = await dbScriptCheckoutCompleteorder(this);
  ...
  async raiseEvent() {
    CompleteorderCheckoutedPublisher.Publish(this.output, this.session).catch(
      (err) => {
  ...
  async afterCheckInstance() {
    try {
      this.checkoutResult = await this.doCheckoutComplete();
    } catch (err) {
      console.log("doCheckoutComplete Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  /***********************************************************************
   ** Complete checkout on Stripe platform, this api will be called to
...
module.exports = CheckoutCompleteOrderManager;
