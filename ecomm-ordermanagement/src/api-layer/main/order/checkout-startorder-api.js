...
const { dbScriptCheckoutStartorder } = require("dbLayer");

class CheckoutStartOrderManager extends OrderManager {
  ...
  async doBusiness() {
    const startorder = await dbScriptCheckoutStartorder(this);
    return startorder;
  }
  ...
  async raiseEvent() {
    StartorderCheckoutedPublisher.Publish(this.output, this.session).catch(
      (err) => {
  ...
  async afterCheckInstance() {
    try {
      this.checkoutResult = await this.doCheckoutStart();
    } catch (err) {
      console.log("doCheckoutStart Action Error:", err.message);
      //**errorLog
      throw err;
    }
  }
  ...
  /***********************************************************************
   ** Start checkout on Stripe platform, this api can be ignored and
   ** checkout start may also be done in frontend.
   ***********************************************************************/
...
module.exports = CheckoutStartOrderManager;
