  ...
  async createPaymentGate() {
    ...
    if (Object.keys(stripeGateWay).length == 0) {
      ...
      if (!publicKey && !secretKey) {
        throw Error(
          "PublicKey and SecretKey For Stripe Shoulşd Be Defined In Project Settings",
        );
      }

      if (!publicKey) {
        throw Error(
          "PublicKey For Stripe Should Be Defined In Project Settings",
        );
      }

      if (!secretKey) {
        throw Error(
          "SecretKey For Stripe Should Be Defined In Project Settings",
        );
      }
    }
