...
console.log("Starting ecomm-ordermanagement-service service...");
...
const start = async () => {
  process.title = "node-ecomm-ordermanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "ecomm-ordermanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key ecomm-ordermanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "ecomm-ordermanagement-service is listening HTTP/REST port " +
...
/* starting the server ecomm-ordermanagement-service .... */
start();
