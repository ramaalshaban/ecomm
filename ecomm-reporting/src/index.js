...
console.log("Starting ecomm-reporting-service service...");
...
const start = async () => {
  process.title = "node-ecomm-reporting-service";
  ...
  try {
    ...
    await setRedisData("ecomm-reporting-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key ecomm-reporting-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "ecomm-reporting-service is listening HTTP/REST port " +
...
/* starting the server ecomm-reporting-service .... */
start();
