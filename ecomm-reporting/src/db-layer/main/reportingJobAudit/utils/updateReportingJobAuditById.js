...
const updateReportingJobAuditById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReportingJobAudit.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReportingJobAudit.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateReportingJobAuditById;
