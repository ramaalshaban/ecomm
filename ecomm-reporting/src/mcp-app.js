...
app.post("/mcp", async (req, res) => {
  ...
  if (sessionIdHeader && sessions[sessionIdHeader]) {
    ...
    const {
      ...
      salesReportMcpRouter,
      exportJobMcpRouter,
      reportingJobAuditMcpRouter,
      getSessionRouter,
    } = require("mcpLayer")(initialHeaders);

    const sessionRouter = getSessionRouter(initialHeaders);

    salesReportMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    exportJobMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
    ...
    reportingJobAuditMcpRouter.forEach((mcpTool) =>
      server.tool(
        mcpTool.name,
        mcpTool.description,
        mcpTool.parameters,
