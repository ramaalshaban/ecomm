...
const updateCartItemById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await CartItem.findOne({ where: { id } });
    ...
    const [rowsCount, [dbDoc]] = await CartItem.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateCartItemById;
