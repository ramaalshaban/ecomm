...
const updateBvfById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Bvf.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Bvf.update(dataClause, options);
    if (!dbDoc) {
