...
// This script is called from the ListCartsManager
...
const dbScriptListCarts = async (apiManager) => {
  ...
  try {
    ...
    let cartList = await Cart.findAll(options);

    const dbData = { items: [] };
    apiManager.carts = [];
    if (!cartList || cartList.length == 0) return dbData;

    dbData.items = Array.isArray(cartList) ? cartList : [cartList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.carts = dbData.items;
    ...
    console.log(err);
    //**errorLog
    throw new HttpServerError("errMsg_dbErrorWhenExecuting_dbScriptListCarts", {
...
module.exports = dbScriptListCarts;
