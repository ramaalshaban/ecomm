...
const updateCartById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Cart.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Cart.update(dataClause, options);
    if (!dbDoc) {
