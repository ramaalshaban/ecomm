...
class GetCartMcpController extends CartMcpController {
  constructor(params) {
    super("getCart", "getcart", params);
    this.dataName = "cart";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetCartManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetCartMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new GetCartMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return GetCartMcpController.getOutputSchema().parse(result);
