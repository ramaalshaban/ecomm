...
console.log("Starting ecomm-cart-service service...");
...
const start = async () => {
  process.title = "node-ecomm-cart-service";
  ...
  try {
    ...
    await setRedisData("ecomm-cart-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key ecomm-cart-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "ecomm-cart-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
...
/* starting the server ecomm-cart-service .... */
start();
