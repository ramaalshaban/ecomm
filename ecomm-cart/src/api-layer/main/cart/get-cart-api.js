...
const { CartRetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetCart } = require("dbLayer");

class GetCartManager extends CartManager {
  ...
  async doBusiness() {
    const cart = await dbScriptGetCart(this);
    return cart;
  }
  ...
  async raiseEvent() {
    CartRetrivedPublisher.Publish(this.output, this.session).catch((err) => {
