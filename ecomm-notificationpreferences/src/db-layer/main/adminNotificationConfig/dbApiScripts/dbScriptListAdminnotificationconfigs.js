...
// This script is called from the ListAdminNotificationConfigsManager
...
const dbScriptListAdminnotificationconfigs = async (apiManager) => {
  ...
  try {
    ...
    let adminNotificationConfigList =
      await AdminNotificationConfig.findAll(options);

    const dbData = { items: [] };
    apiManager.adminNotificationConfigs = [];
    if (!adminNotificationConfigList || adminNotificationConfigList.length == 0)
      return dbData;

    dbData.items = Array.isArray(adminNotificationConfigList)
      ? adminNotificationConfigList
      : [adminNotificationConfigList];
    ...
    apiManager.adminNotificationConfigs = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListAdminnotificationconfigs",
      {
...
module.exports = dbScriptListAdminnotificationconfigs;
