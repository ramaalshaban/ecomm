...
// This script is called from the UpdateAdminNotificationConfigManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.adminNotificationConfig;
  return {
    old_adminNotificationConfig: oldData,
    adminNotificationConfig: newData,
...
const dbScriptUpdateAdminnotificationconfig = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await AdminNotificationConfig.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateAdminnotificationconfig",
      {
...
module.exports = dbScriptUpdateAdminnotificationconfig;
