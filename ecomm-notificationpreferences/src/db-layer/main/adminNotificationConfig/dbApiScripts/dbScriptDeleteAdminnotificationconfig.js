...
// This script is called from the DeleteAdminNotificationConfigManager
...
const dbScriptDeleteAdminnotificationconfig = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await AdminNotificationConfig.update(
      { isActive: false },
      {
        where: whereClause,
        returning: true,
    ...
    apiManageradminNotificationConfig = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeleteAdminnotificationconfig",
      {
...
module.exports = dbScriptDeleteAdminnotificationconfig;
