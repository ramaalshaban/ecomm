...
// This script is called from the UpdateUserNotificationPreferencesManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.userNotificationPreferences;
  return {
    old_userNotificationPreferences: oldData,
    userNotificationPreferences: newData,
...
const dbScriptUpdateUsernotificationpreferences = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await UserNotificationPreferences.update(
      dataClause,
      {
        where: whereClause,
        returning: true,
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateUsernotificationpreferences",
      {
...
module.exports = dbScriptUpdateUsernotificationpreferences;
