...
// This script is called from the ListUserNotificationPreferencesManager
...
const dbScriptListUsernotificationpreferences = async (apiManager) => {
  ...
  try {
    ...
    let userNotificationPreferencesList =
      await UserNotificationPreferences.findAll(options);

    const dbData = { items: [] };
    apiManager.userNotificationPreferencess = [];
    if (
      !userNotificationPreferencesList ||
      userNotificationPreferencesList.length == 0
    )
      return dbData;

    dbData.items = Array.isArray(userNotificationPreferencesList)
      ? userNotificationPreferencesList
      : [userNotificationPreferencesList];
    ...
    apiManager.userNotificationPreferencess = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListUsernotificationpreferences",
      {
...
module.exports = dbScriptListUsernotificationpreferences;
