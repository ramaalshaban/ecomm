...
const updateUserNotificationPreferencesById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await UserNotificationPreferences.findOne({
    ...
    const [rowsCount, [dbDoc]] = await UserNotificationPreferences.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateUserNotificationPreferencesById;
