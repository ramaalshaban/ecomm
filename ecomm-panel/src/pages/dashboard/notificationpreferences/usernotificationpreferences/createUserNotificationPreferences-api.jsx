...
export default function NotificationPreferencesCreateUserNotificationPreferencesApiPage() {
  const [view, setView] = useState("Table");
  const [
    createdUsernotificationpreferences,
    setCreatedUsernotificationpreferences,
  ] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedUsernotificationpreferences(null);
      reset();
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(createdUsernotificationpreferences || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={createdUsernotificationpreferences}
                error={error}
              />
            ) : (
              <JsonResponse
                content={createdUsernotificationpreferences || error}
              />
            )}
          </Box>
        </Box>
