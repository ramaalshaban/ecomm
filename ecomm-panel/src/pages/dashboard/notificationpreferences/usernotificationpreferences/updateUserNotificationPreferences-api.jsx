...
export default function NotificationPreferencesUpdateUserNotificationPreferencesApiPage() {
  const [view, setView] = useState("Table");
  const [
    updatedUsernotificationpreferences,
    setUpdatedUsernotificationpreferences,
  ] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedUsernotificationpreferences(null);
      reset();
  ...
  return (
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      <Divider />
      {(updatedUsernotificationpreferences || error) && (
        <Box paddingTop="2rem">
          <Box
            display="flex"
            flexDirection="row"
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse
                content={updatedUsernotificationpreferences}
                error={error}
              />
            ) : (
              <JsonResponse
                content={updatedUsernotificationpreferences || error}
              />
            )}
          </Box>
        </Box>
