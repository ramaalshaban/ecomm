...
// This script is called from the ListProductsManager
...
const dbScriptListProducts = async (apiManager) => {
  ...
  try {
    ...
    let productList = await Product.findAll(options);

    const dbData = { items: [] };
    apiManager.products = [];
    if (!productList || productList.length == 0) return dbData;

    dbData.items = Array.isArray(productList) ? productList : [productList];
    dbData.items = dbData.items.map((item) => item.getData());

    dbData.totalRowCount = dbData.items.length;
    dbData.pageCount = 1;

    apiManager.products = dbData.items;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptListProducts",
      {
...
module.exports = dbScriptListProducts;
