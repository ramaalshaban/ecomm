...
// This script is called from the UpdateProductManager
...
async function getEventPayload(apiManager, dataClause) {
  ...
  const newData = apiManager.product;
  return {
    old_product: oldData,
    product: newData,
...
const dbScriptUpdateProduct = async (apiManager) => {
  ...
  try {
    ...
    let rowsCount = null;
    let dbDoc = null;
    [rowsCount, [dbDoc]] = await Product.update(dataClause, {
      where: whereClause,
      returning: true,
    });
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptUpdateProduct",
      {
...
module.exports = dbScriptUpdateProduct;
