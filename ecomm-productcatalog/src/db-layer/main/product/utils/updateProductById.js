...
const updateProductById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Product.findOne({ where: { id } });
    ...
    const [rowsCount, [dbDoc]] = await Product.update(dataClause, options);
    if (!dbDoc) {
