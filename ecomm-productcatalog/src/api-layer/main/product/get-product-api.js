...
const { ProductRetrivedPublisher } = require("../../api-events/publishers");
...
const { dbScriptGetProduct } = require("dbLayer");

class GetProductManager extends ProductManager {
  ...
  async doBusiness() {
    const product = await dbScriptGetProduct(this);
    return product;
  }
  ...
  async raiseEvent() {
    ProductRetrivedPublisher.Publish(this.output, this.session).catch((err) => {
