...
console.log("Starting ecomm-productcatalog-service service...");
...
const start = async () => {
  process.title = "node-ecomm-productcatalog-service";
  ...
  try {
    ...
    await setRedisData(
      "ecomm-productcatalog-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key ecomm-productcatalog-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "ecomm-productcatalog-service is listening HTTP/REST port " +
...
/* starting the server ecomm-productcatalog-service .... */
start();
