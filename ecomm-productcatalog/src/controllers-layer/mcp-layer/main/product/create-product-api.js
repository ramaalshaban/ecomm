...
class CreateProductMcpController extends ProductCatalogMcpController {
  constructor(params) {
    super("createProduct", "createproduct", params);
    this.dataName = "product";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateProductManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      images: z.string().max(255).describe("Array of product image URLs."),
...
module.exports = (headers) => {
  return {
    ...
    parameters: CreateProductMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CreateProductMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CreateProductMcpController.getOutputSchema().parse(result);
