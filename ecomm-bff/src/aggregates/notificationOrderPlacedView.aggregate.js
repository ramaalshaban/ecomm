...
const getAllNotificationOrderPlacedView = async (filter = null) => {
  try {
...
const getNotificationOrderPlacedView = async (id) => {
  try {
...
const productsAggregateDataFromIndex = async (source) => {
  if (!source["orderItems"]) return;
  const aggregation = await elasticClient.search({
    index: "ecomm_product",
    body: {
      query: {
        match: {
          id: source["orderItems"],
        },
      },
      _source: ["id", "name", "images", "category"],
    },
  });
