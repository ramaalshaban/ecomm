...
const consumer = kafka.consumer({
  groupId: `ecomm-notification-service-email-consumer`,
});
...
const runEmailSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `ecomm-notification-service-notification-email`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(`ecomm-notification-service-notification-email: `, error);
      }
    },
  });
};
