...
const consumer = kafka.consumer({
  groupId: `ecomm-notification-service-sms-consumer`,
});
...
const runSmsSenderListener = async () => {
  ...
  await consumer.subscribe({
    topic: `ecomm-notification-service-notification-sms`,
    fromBeginning: true,
  });
  ...
  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        ...
        //**errorLog
        console.error(`ecomm-notification-service-notification-sms: `, error);
      }
    },
  });
};
