...
console.log("Starting ecomm-auth-service service...");
...
const EcommLoginSession = require("./project-session/ecomm-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-ecomm-auth-service";
  ...
  try {
    ...
    await setRedisData("ecomm-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key ecomm-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "ecomm-auth-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
  ...
  // init login database
  const loginSession = new EcommLoginSession();
...
/* starting the server ecomm-auth-service .... */
start();
