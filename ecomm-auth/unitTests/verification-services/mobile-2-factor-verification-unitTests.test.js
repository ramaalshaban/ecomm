...
describe("Mobile2Factor Service", function () {
  ...
  beforeEach(() => {
    // fresh stubs for Redis each test
    getRedisDataStub = sinon.stub().resolves(null);
    setRedisDataStub = sinon.stub().resolves();

    ({ Mobile2Factor } = proxyquire(
      "../../src/verification-services/mobile-2-factor-verification",
      {
        common: {
          ForbiddenError,
  ...
  describe("complete2Factor", () => {
    ...
    it("should throw NotAuthenticatedError if session is not found", async () => {
      ...
      getRedisDataStub.resolves(null); // force session not found
    ...
    it("should successfully complete mobile 2FA when the correct code is provided", async () => {
      ...
      getRedisDataStub.resolves(sessionData);
